...
# ── CRITICAL: Set Qt platform BEFORE any PySide6 import ─────────────────────
os.environ['QT_QPA_PLATFORM'] = 'offscreen'
os.environ['QT_QPA_PLATFORM_PLUGIN_PATH'] = ''  # Use default
...
def qapp():
    ...
    Must be session-scoped because only ONE QApplication can exist per process.
    The 'offscreen' platform plugin renders to memory (no display needed).
    """
    ...
    if _qapp_instance is None:
        ...
        _qapp_instance.setApplicationName("Chess Puzzle Studio Tests")
    yield _qapp_instance


# ── Process Qt events helper ────────────────────────────────────────────────

@pytest.fixture
def process_events(qapp):
    """Provide a helper to process pending Qt events."""
    def _process():
        qapp.processEvents()
    return _process
