...
class TestCoordinateConversion:
    """Test the coordinate conversion helper functions — no Qt needed."""
...
class TestRenderFrame:
    """Test the static render_frame method — requires QApplication."""

    def test_renders_basic_board(self, qapp, process_events):
    ...
    def test_renders_with_custom_sq_size(self, qapp, process_events):
        from board_widget import ChessBoardWidget
        board = chess.Board()
    ...
    def test_renders_with_animation(self, qapp):
        ...
        anim = {
            "from": (6, 4), "to": (4, 4),
            "piece_obj": chess.Piece(chess.PAWN, chess.WHITE),
            "progress": 0.5, "captured": ".",
        }
    ...
    def test_renders_with_text_overlay(self, qapp):
        ...
        board = chess.Board()
        img = ChessBoardWidget.render_frame(
            board, text_overlay="White to play")
        assert isinstance(img, QImage)
    ...
    def test_renders_without_arrow(self, qapp):
        ...
        board = chess.Board()
        img = ChessBoardWidget.render_frame(
            board, last_move=((6, 4), (4, 4)), show_arrow=False)
        assert isinstance(img, QImage)
...
class TestRenderLayout:
    def test_board_only_layout(self, qapp):
        ...
        result = ChessBoardWidget.render_layout(
            board_img, ["e4"], 0, LayoutMode.BOARD_ONLY,
            544, 544, (26, 27, 46), SQ_SIZE)
    ...
    def test_board_moves_right_layout(self, qapp):
        ...
        board_img = ChessBoardWidget.render_frame(board)
        result = ChessBoardWidget.render_layout(
            board_img, ["e4", "e5"], 0, LayoutMode.BOARD_MOVES_RIGHT,
            800, 544, (26, 27, 46), SQ_SIZE)
        assert isinstance(result, QImage)
    ...
    def test_board_moves_bottom_layout(self, qapp):
        ...
        board_img = ChessBoardWidget.render_frame(board)
        result = ChessBoardWidget.render_layout(
            board_img, ["e4", "e5"], 0, LayoutMode.BOARD_MOVES_BOTTOM,
            544, 700, (26, 27, 46), SQ_SIZE)
        assert isinstance(result, QImage)
    ...
    def test_move_list_visible_false(self, qapp):
        ...
        board_img = ChessBoardWidget.render_frame(board)
        result = ChessBoardWidget.render_layout(
            board_img, ["e4"], 0, LayoutMode.BOARD_MOVES_RIGHT,
            800, 544, (26, 27, 46), SQ_SIZE, move_list_visible=False)
        assert isinstance(result, QImage)
...
class TestRenderCard:
    ...
    def test_card_custom_colors(self, qapp):
        from board_widget import ChessBoardWidget
        img = ChessBoardWidget.render_card(
            "Test", bg_color="#000000", fg_color="#ffffff")
        assert isinstance(img, QImage)
...
class TestChessBoardWidget:
    """Tests for the live widget — requires QApplication and careful cleanup."""

    def test_widget_creation(self, qapp, process_events):
        from board_widget import ChessBoardWidget
    ...
    def test_flip(self, qapp, process_events):
        from board_widget import ChessBoardWidget
        from chess_engine import ChessEngine
        from sound_manager import SoundManager
        engine = ChessEngine()
    ...
    def test_set_legal_targets(self, qapp, process_events):
        from board_widget import ChessBoardWidget
        from chess_engine import ChessEngine
        from sound_manager import SoundManager
        engine = ChessEngine()
    ...
    def test_start_animation(self, qapp, process_events):
        from board_widget import ChessBoardWidget
        from chess_engine import ChessEngine
        from sound_manager import SoundManager
        engine = ChessEngine()
        ...
        widget = ChessBoardWidget(engine, snd)
        process_events()
        widget.start_animation(6, 4, 4, 4,
                              chess.Piece(chess.PAWN, chess.WHITE),
                              notation="e4")
        assert widget.animating is True
        # Stop the animation timer to avoid interference
        widget._anim_timer.stop()
