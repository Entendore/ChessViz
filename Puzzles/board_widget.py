...
class ChessBoardWidget(QWidget):
    ...
    # FIX: end the QPainter explicitly
    def paintEvent(self, e):
        ...
        img = self.render_frame(self.engine.board, self.engine.last_move,
                                ...
                                check_squares=chk,
                                anim_state=self._get_anim_state(),
    ...
    def render_frame(board, last_move=None, selected=None, legal_targets=None,
        ...
        # IMPROVEMENT: draw captured piece fading out at destination during animation
        if anim_state and anim_state.get('captured', '.') != '.':
            ...
            if cap_piece is None:
                # The captured piece was already removed from the board copy;
                # reconstruct it from the captured symbol
                ...
                if pt:
                    cap_piece = chess.Piece(pt,
                                            chess.WHITE if is_w else chess.BLACK)
                    fade = max(0, int(200 * (1.0 - anim_state['progress'])))
                    p.setOpacity(fade / 255.0)
                    ChessBoardWidget._draw_piece(p, cap_piece, tr, tc_, sz,
                                                 font_piece)
                    p.setOpacity(1.0)
    ...
    def _draw_arrow(painter, fx, fy, tx, ty, color, sz):
        ...
        tri = QPolygonF([QPointF(tx2, ty2), QPointF(p1x, p1y),
                         QPointF(p2x, p2y)])
    ...
    def mousePressEvent(self, e):
        ...
        if self.selected:
            ...
            if (r, c) in self.legal_targets:
                ...
                if info:
                    # FIX: also detect en passant as capture for sound
