...
class SoundManager:
    ...
    def _sin(f, d, v=.5, sr=44100):
        return [32767 * v * math.sin(2 * math.pi * f * i / sr)
                for i in range(int(sr * d))]

    @staticmethod
    def _env(s, a=.01, r=.02, sr=44100):
        o = list(s); ai = int(sr * a); ri = int(sr * r)
        for i in range(min(ai, len(o))):
            o[i] *= i / ai
        for i in range(min(ri, len(o))):
            o[-(i + 1)] *= i / ri
        return o
    ...
    def _gen_all(self):
        ...
        self._wav(os.path.join(d, "move.wav"),
                  self._env(self._sin(800, .06, .4), .005, .03))

        self._wav(os.path.join(d, "capture.wav"),
                  self._env(self._mix(self._sin(300, .10, .5),
                                      self._sin(600, .08, .3)), .005, .04))

        self._wav(os.path.join(d, "check.wav"),
                  self._env(self._mix(self._sin(1000, .12, .5),
                                      self._sin(1250, .10, .3)), .005, .04))

        self._wav(os.path.join(d, "checkmate.wav"),
                  self._env(self._sin(800, .15, .5) + self._sin(600, .15, .5)
                            + self._sin(400, .25, .5), .01, .08))

        n = int(sr * .15)
        sw = [32767 * .4 * math.sin(2 * math.pi * (400 + 400 * i / n) * i / sr)
              for i in range(n)]
        self._wav(os.path.join(d, "castle.wav"), self._env(sw, .005, .03))

        self._wav(os.path.join(d, "error.wav"),
                  self._env(self._sin(200, .10, .4), .005, .03))

        n2 = int(sr * .2)
        ri = [32767 * .4 * math.sin(2 * math.pi * (400 + 400 * i / n2) * i / sr)
              for i in range(n2)]
    ...
    # ── Load into QSoundEffect objects ────────────────────────────────────────
    def _load_all(self):
