"""Chess Video Maker Pro — Thread-Safe Board Renderer

Standalone chess board renderer that produces QImage frames without
depending on any QWidget.  Safe to call from any thread (QPainter on
QImage is explicitly supported by Qt in non-GUI threads).
"""
import math
import chess
...
class BoardRenderer:
    ...
    def __init__(self, board=None, theme=None, flipped=False, show_coords=True):
        ...
        self.arrows: list = []
        # Animation state (set by video pipeline, not used for live widget)
        self.anim_move = None
    ...
    def from_widget(cls, widget):
        """Snapshot a ChessBoardWidget's visual state for thread-safe rendering."""
        r = cls()
        r.board = widget.board.copy()
        r.theme = widget.theme
        r.flipped = widget.flipped
        r.show_coords = widget.show_coords
        r.last_move = widget.last_move
        r.highlighted = set(widget.highlighted)
        r.arrows = list(widget.arrows)
        r.policy_vis = dict(widget.policy_vis)
        return r
    ...
    def _paint(self, p, t, m, sz):
        ...
        for s in chess.SQUARES:
            ...
            if self.last_move and s in (self.last_move.from_square,
        ...
        for mv in self.legal_targets:
            ...
            if self.board.piece_at(mv):
                ...
                p.drawEllipse(rect.adjusted(sz * 0.1, sz * 0.1,
                                            -sz * 0.1, -sz * 0.1))
            else:
            ...
            for uci, pr in self.policy_vis.items():
                try:
                    ...
                    if mv in self.board.legal_moves:
                        ...
                        p.setBrush(QColor.fromHsvF(
                            0.33 * pr, 0.9, 0.9, 0.6 * pr + 0.1))
