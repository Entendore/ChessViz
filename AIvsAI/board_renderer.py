...
Performance: pre-cached QFont/QPen objects avoid per-frame allocation.
"""
...
from constants import (
    ...
    MQ_BRILLIANT, MQ_BLUNDER, MQ_SHOW_BADGE,
)
...
class BoardRenderer:
    def __init__(self, board=None, theme=None, flipped=False):
        ...
        self.theme = theme or __import__("constants").BoardTheme()
        ...
        # ── Pre-cached rendering objects (updated per frame) ──
        self._cache_sz = -1
    ...
    def _update_cache(self, sz):
        """Rebuild cached fonts/pens when square size changes."""
        isz = int(sz * 100)  # quantise to avoid float drift
        ...
        self._font_badge_normal = QFont("Inter", max(6, int(sz * 0.19 * 0.95)), QFont.Bold)
        self._font_badge_symbol = QFont("Segoe UI Symbol", max(7, int(sz * 0.19 * 1.15)), QFont.Bold)
        ...
        self._pen_badge_outline = QPen(QColor(255, 255, 255, 120), max(0.8, sz * 0.008))
    ...
    def _draw_quality_badge(self, p, sq, t, m, sz):
        ...
        r = sz * 0.19
        cx = rect.right() - r - sz * 0.06
        cy = rect.top() + r + sz * 0.06
        center = QPointF(cx, cy)

        # ── Glow effect ───────────────────────────────────
        if quality == MQ_BRILLIANT:
            glow = QRadialGradient(center, r * 3.0)
            glow.setColorAt(0.0, QColor(0, 210, 175, 90))
            glow.setColorAt(0.5, QColor(0, 210, 175, 35))
            glow.setColorAt(1.0, QColor(0, 210, 175, 0))
            p.setPen(Qt.NoPen)
            p.setBrush(glow)
            p.drawEllipse(QRectF(cx - r * 3.0, cy - r * 3.0, r * 6.0, r * 6.0))
        elif quality == MQ_BLUNDER:
            glow = QRadialGradient(center, r * 3.0)
            glow.setColorAt(0.0, QColor(220, 45, 45, 90))
            glow.setColorAt(0.5, QColor(220, 45, 45, 35))
            glow.setColorAt(1.0, QColor(220, 45, 45, 0))
            p.setPen(Qt.NoPen)
            p.setBrush(glow)
            p.drawEllipse(QRectF(cx - r * 3.0, cy - r * 3.0, r * 6.0, r * 6.0))
        ...
        p.drawEllipse(QRectF(cx - r + 1, cy - r + 1.5, 2 * r, 2 * r))
    ...
    # ── Piece drawing (uses cached fonts/pens) ────────────────
