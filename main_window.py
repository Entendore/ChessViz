...
from constants import (AI_MAP, THEMES, BoardTheme, HAS_CV2,
                       ...
                       get_system_ram_gb, estimate_memory_gb, find_stockfish)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self._update_mem_estimate(); self._new_game()
    ...
    def _update_mem_estimate(self):
        try:
            ...
            fps = self.fps_spin.value(); hold = self.hold_spin.value()
            moves = max(1, len(self.move_list))
            est = estimate_memory_gb(res_str, fps, hold, moves)
            ram = get_system_ram_gb()
            frame_count = max(1, int(hold * fps)) * (moves + 1)
            if est > ram * 0.6:
                color, note = "#e55", "⚠️ Exceeds 60% RAM — enable Disk Cache!"
            elif est > ram * 0.3:
                color, note = "#c90", "⚠️ Uses >30% RAM — consider Disk Cache"
            else:
                color, note = "#6b6", "✅ Comfortable"
            self.mem_estimate_lbl.setText(f"~{est:.1f} GB for ~{frame_count} frames  {note}")
            self.mem_estimate_lbl.setStyleSheet(f"color:{color};font-size:11px;padding:2px")
        except: self.mem_estimate_lbl.setText("Est. memory: —")

    def _on_quality_preset(self, idx):
        if idx < 0: return
        name = self.quality_preset_combo.currentData()
        if name not in QUALITY_PRESETS: return
        p = QUALITY_PRESETS[name]
        for w in (self.export_res_combo, self.export_fps_spin,
                  self.fps_spin, self.hold_spin, self.disk_cache_chk):
            w.blockSignals(True)
    ...
    def _on_disk_cache_toggled(self, checked):
        self._use_disk_cache = checked; self._update_mem_estimate()
    ...
    def _load_pgn_data(self, g):
        ...
        self.eval_bar_widget.set_eval(0.0); self._update_mem_estimate()
    ...
    def _on_sq_click(self, sq):
        ...
        if self.board_widget.selected_sq is None:
            ...
            if mv in b.legal_moves:
                ...
                self._play_sound_for_move(self.node); self._update_mem_estimate()
            else:
                self.board_widget.selected_sq = None
                self.board_widget.legal_targets = []
    ...
    def _browse_batch_pgn_folder(self):
        ...
        if d:
            ...
            # Default output folder to subfolder
    ...
    def _browse_batch_output_folder(self):
        ...
        if d:
            self.batch_output_folder_edit.setText(d)

    def _start_batch_pgn_export(self):
        """Start batch rendering all PGN files in a folder."""
        ...
        if not pgn_dir or not os.path.isdir(pgn_dir):
            ...
            self.batch_status_lbl.setStyleSheet("color:#e55;font-size:11px")
            return
        ...
        # Gather settings
        preset_name = self.batch_quality_combo.currentData()
        preset = QUALITY_PRESETS.get(preset_name, QUALITY_PRESETS["Medium"])
        res_str = RESOLUTION_LIST[preset["resolution_index"]]
        ...
        settings = {
            ...
            "stockfish_path": self.engine_path_edit.text()
        }
    ...
    def _on_batch_game_exported(self, output_path):
        """Called when a single game in the batch finishes successfully."""
    ...
    def _on_sound_design(self, d):
        ...
        self.sound_design_desc.setText(_SOUND_DESIGN_DESC.get(d, f"🎵 {d}"))
    def _on_snd_type_vol(self, t, v): self.sound_manager.set_type_volume(t, v)
    def _test_sound(self, t): self.sound_manager.play(t)
    def _on_anim_enabled(self, e): self.anim_manager.enabled = e
    def _on_piece_anim(self, e): self.anim_manager.set_piece_anim(e)
    def _on_highlight_anim(self, e): self.anim_manager.set_highlight_anim(e)
    def _on_eval_anim(self, e): self.anim_manager.set_eval_anim(e)
    def _on_anim_dur(self, ms): self.anim_manager.set_duration(ms)
    def _on_anim_ease(self, n): self.anim_manager.set_easing(n)

    # ════════════════════════════════════════════════════════════════
    #  NEW: Pipeline — Quick PGN → MP4
    # ════════════════════════════════════════════════════════════════
    def _quick_load_pgn(self):
        """File dialog to load a PGN for the pipeline."""
    ...
    def _quick_pgn_to_mp4(self):
        """One-click PGN → MP4 pipeline using streaming export."""
        ...
        # Get quality preset
        preset_name = self.quick_quality_combo.currentData()
        preset = QUALITY_PRESETS.get(preset_name, QUALITY_PRESETS["Medium"])

        res_str = RESOLUTION_LIST[preset["resolution_index"]]
        fps = preset["fps"]
        hold = self.quick_hold_spin.value()
        output = os.path.join(self.mp4_output_dir, self.quick_output_edit.text().strip() or "chess_pgn_video.mp4")

        # Create thread-safe board renderer snapshot
        br = BoardRenderer.from_widget(self.board_widget)

        sf_path = self.engine_path_edit.text() if self.quick_eval_chk.isChecked() else ""
    ...
    # ════════════════════════════════════════════════════════════════
    #  NEW: Pipeline — Quick AI → MP4
    # ════════════════════════════════════════════════════════════════
    def _quick_ai_to_mp4(self):
        """One-click AI Battle → MP4 pipeline.

        Step 1: Run AI battle in background thread (AIBattleWorker)
        Step 2: On completion, load PGN and start streaming export
        """
        ...
        self.quick_ai_status_lbl.setText("⚔️ Battle in progress…")
    ...
    def _ai_params(self, et, strength):
        ...
        if et == "Minimax (Alpha-Beta)":
            ...
            par["path"] = self.engine_path_edit.text()
        return par

    def _on_pipeline_battle_move(self, uci, eval_cp):
        """Live update of the board as the pipeline battle plays."""
        try:
            mv = chess.Move.from_uci(uci)
            if self.node and mv in self.board_widget.board.legal_moves:
                self.node = self.node.add_variation(mv)
                self.move_list = list(self.game.mainline())
                self.move_index += 1
                self._update_board(animate=True, move_obj=mv)
    ...
    def _on_pipeline_battle_progress(self, move_num, max_moves):
        pct = int(move_num / max_moves * 50)  # First 50% is battle
        self.quick_ai_progress_bar.setValue(pct)
        self.quick_ai_status_lbl.setText(f"⚔️ Move {move_num}/{max_moves}")

    def _on_pipeline_battle_done(self, pgn_text, result):
        """Battle finished — now start streaming export."""
        ...
        preset_name = self.quick_ai_quality_combo.currentData()
        preset = QUALITY_PRESETS.get(preset_name, QUALITY_PRESETS["Medium"])
        res_str = RESOLUTION_LIST[preset["resolution_index"]]
        fps = preset["fps"]
    ...
    def _on_quick_ai_export_progress(self, pct, txt):
        # Scale to 50-100% range (battle was 0-50%)
    ...
    def _run_engine(self):
        ...
        elif et == "Stockfish (UCI)": pa["path"] = self.engine_path_edit.text()
    ...
    def _auto_export(self):
        ...
        self.statusBar().showMessage("🎬 Rendering…"); QApplication.processEvents()
        self._auto_capture()
        if dp: self._save_png()
    ...
    def _auto_capture(self):
        ...
        disk_dir = self._disk_cache_dir if use_disk else None
        ...
        self._capture_worker = CaptureWorker(
            ...
            black_name=self.black_name_edit.text(),
            overlays=list(self.canvas_overlays),
            fps=fps, hold=self.hold_spin.value(),
            res_str=self.export_res_combo.currentText(),
        ...
        self.statusBar().showMessage(
            f"🎬 Capturing {estimated} frames ({'disk' if use_disk else 'memory'})…")
    ...
    def _on_capture_finished(self, success):
        ...
        if self._capture_worker:
            if self._capture_worker.use_disk_cache:
                self._disk_cache_dir = self._capture_worker.disk_cache_dir
                self._disk_frame_count = self._capture_worker.disk_frame_count
                self._use_disk_cache = True
            else:
                self.capture_frames = self._capture_worker.captured_images

        total = self._disk_frame_count if self._use_disk_cache else len(self.capture_frames)
        self.frame_count_lbl.setText(f"Frames: {total}")
        self.statusBar().showMessage(f"✅ Captured {total} frames")
    ...
    def _clear_frames(self):
        self.capture_frames = []; self._cleanup_disk_cache()
        self.frame_count_lbl.setText("Frames: 0"); self._cleanup_preview()

    def _start_inline_export(self):
        try:
            if not HAS_CV2:
                self.export_status_lbl.setText("ERROR: OpenCV not available"); return
            has_memory = bool(self.capture_frames)
            has_disk = (self._disk_cache_dir is not None and self._disk_frame_count > 0)
            if not has_memory and not has_disk:
                self.export_status_lbl.setText("ERROR: No frames to export"); return
            ...
            if not out:
                self.export_status_lbl.setText("ERROR: No output path"); return
            ...
            if has_memory:
                for fi, f in enumerate(self.capture_frames):
                    try:
                        ...
                        arr = _qimage_to_bgr_numpy(f)
                        if arr is not None: npf.append(arr)
                    except: continue
                if not npf and not has_disk:
                    self.export_status_lbl.setText("ERROR: No valid frames")
                    return
            ...
            self.export_status_lbl.setText("Exporting..."); self.export_worker.start()
        except Exception as e:
    ...
    def _start_streaming_export(self):
        """Stream export — render and export in one pass (constant memory)."""
        if not HAS_CV2:
            self.export_status_lbl.setText("ERROR: OpenCV not available"); return
        if not self.move_list:
            self.export_status_lbl.setText("ERROR: No moves to export"); return
        ...
        self._streaming_worker = StreamingExportWorker(
            ...
            overlays=list(self.canvas_overlays),
            fps=fps, hold=self.hold_spin.value(),
            res_str=res_str, output_path=out)
        self._streaming_worker.progress.connect(self._on_export_prog)
        self._streaming_worker.export_finished.connect(self._on_export_done)
        self.export_start_btn.setEnabled(False); self.export_cancel_btn.setEnabled(True)
        self.export_status_lbl.setText("Streaming export...")
        self._streaming_worker.start()

    def _on_export_prog(self, pct, txt):
    ...
    def _on_export_done(self, msg):
        ...
        if msg.startswith("Done"):
            ...
            self.export_status_lbl.setText("⏹ Export cancelled")
        else:
            self.export_status_lbl.setText(msg)

    def _cancel_export(self):
        if self.export_worker: self.export_worker.cancel()
        if self._streaming_worker: self._streaming_worker.cancel()

    # ── QImage → numpy helper ──────────────────────────────────────
    def _qimage_to_numpy(self, qimg):
        return _qimage_to_bgr_numpy(qimg)