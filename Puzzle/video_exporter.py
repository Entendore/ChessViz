...
from utils import log, ease_out_cubic, sanitize_filename, HAS_FFMPEG, HAS_CUPY
...
def _generate_silent_wav(path, duration_s, sr=44100):
    """Generate a silent WAV file of the given duration for YouTube-compatible output."""
...
class FFmpegVideoExporter(QObject):
    ...
    error = Signal(str)
    log_msg = Signal(str)
    batch_puzzle_done = Signal(int, int, str)
    ...
    def export_puzzle(self, puzzle, output_path):
        """Export a single puzzle to an MP4 video file."""
        if not HAS_FFMPEG:
    ...
    def export_puzzle_threaded(self, puzzle, output_path):
        """Export a single puzzle in a background thread."""
        import threading
    ...
    def export_batch(self, puzzles, output_dir):
        """Export multiple puzzles to MP4 files in a background thread."""
        import threading
        if not HAS_FFMPEG:
    ...
    def _add_silent_audio(self, video_path, actual_frames):
        """Add a silent audio track to the video for YouTube compatibility."""
    ...
    def _export_streaming(self, puzzle, output_path):
        ...
        try:
            ...
            for i in range(setup_count):
                ...
                frame_idx = self._execute_move_streaming(
                    process, engine, uci_moves[i], san_moves, i,
                    frame_idx, total_est,
                    sq_size, w, h, cfg, theme, puzzle,
                    "Setting up...", is_setup=True)

            # ── 3. Starting Position Hold Phase ──────────────────────────
            if cfg.position_hold_enabled:
                n_hold = max(1, int(cfg.fps * cfg.position_hold_duration))
                hold_last_move = engine.last_move
            ...
            for loop_idx in range(loops):
                if loop_idx > 0:
                    ...
                    for i in range(setup_count):
                        if i < len(uci_moves):
                            engine.make_move_uci(uci_moves[i])
            ...
            if cfg.end_enabled:
                ...
                end_img = self._render_composited_frame(
                    engine.board, theme, sq_size, w, h, cfg, san_moves,
                    len(uci_moves) - 1, puzzle, "",
                    last_move=end_last_move,
                    text_overlay=end_overlay)
        ...
        log("FFmpeg wrote video successfully", "EXPORT")
        return frame_idx
    ...
    def _is_key_move(self, engine, uci_str):
        try:
            ...
            if engine.board.is_capture(move):
                return True
            if engine.board.gives_check(move):
                return True
    ...
    def _render_composited_frame(self, board, theme, sq_size, w, h, cfg,
                                 san_moves, current_move_idx, puzzle_info,
                                 status_text, anim_state=None, last_move=None,
                                 text_overlay=""):
        """Render a full composited frame: board + optional move list panel."""
        check_sqs = []
        if board.is_check():
        ...
        final_img = ChessBoardWidget.render_layout(
            ...
            cfg.layout_mode, w, h, cfg.background_color, sq_size,
            puzzle_info, status_text, cfg.move_list_visible)
        return final_img
    ...
    def _estimate_frame_count(self, n_moves):
        ...
        if cfg.position_hold_enabled:
            total += int(cfg.fps * cfg.position_hold_duration)
        loops = max(1, cfg.loop_count)
        anim_frames = max(1, int(cfg.fps * cfg.move_anim_duration))
        pause_frames = max(1, int(cfg.fps * cfg.pause_after_move))
        total += loops * n_moves * (anim_frames + pause_frames)
        if cfg.end_enabled:
