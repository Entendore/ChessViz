#!/usr/bin/env python3
"""
Chess Openings App — Single-file PySide6 Application
...
import sys, os, math, time, csv, re, ast, base64, threading, shutil, tempfile, gc, wave, subprocess, json, sqlite3
import chess
import numpy as np
...
def parse_opening_image(img_val):
    ...
    if isinstance(img_val, dict): img_dict = img_val
        ...
        try:
            safe = img_val; safe = re.sub(r'\bnull\b', 'None', safe)
            safe = re.sub(r'\btrue\b', 'True', safe); safe = re.sub(r'\bfalse\b', 'False', safe)
            safe = re.sub(r'\bNaN\b', 'None', safe); safe = re.sub(r'\bundefined\b', 'None', safe)
...
class ChessEngine:
    ...
    @property
    def turn(self): return 'w' if self.board.turn == chess.WHITE else 'b'
    def color_of(self, piece): return 'w' if piece.color == chess.WHITE else 'b'
    def check_squares(self):
        if self.board.is_check(): return [self.sq_to_rc(self.board.king(self.board.turn))]
        return []
    ...
    def make_move(self, fr, fc, tr, tc, promo=None):
        ...
        if piece.piece_type == chess.PAWN:
            if (piece.color == chess.WHITE and tr == 0) or (piece.color == chess.BLACK and tr == 7):
                if promo: promotion = chess.PIECE_SYMBOLS.index(promo.lower()) + 1
                else: promotion = chess.QUEEN
        move = chess.Move(from_sq, to_sq, promotion=promotion)
        if move not in self.board.legal_moves: return None
        is_castle = self.board.is_castling(move); is_ep = self.board.is_en_passant(move)
        if is_ep:
            ep_cap_sq = chess.square(chess.square_file(to_sq), chess.square_rank(from_sq))
            cap = self.board.piece_at(ep_cap_sq)
        else: cap = self.board.piece_at(to_sq)
        captured = cap.symbol() if cap else '.'
        notation = self.board.san(move); piece_obj = chess.Piece(piece.piece_type, piece.color)
        self.board.push(move); self.last_move = ((fr, fc), (tr, tc))
        self.game_over = self.board.is_game_over()
        self.result = self.board.result() if self.game_over else ""
        return {'from': (fr, fc), 'to': (tr, tc), 'piece': piece.symbol(),
    ...
    def undo(self):
        if len(self.board.move_stack) > 0:
            self.board.pop(); self.game_over = self.board.is_game_over()
            self.result = self.board.result() if self.game_over else ""
    ...
    def load_fen(self, fen):
        self.board.set_fen(fen); self.game_over = self.board.is_game_over()
        self.result = self.board.result() if self.game_over else ""
        self.last_move = None
...
if HAS_NUMBA:
    ...
    def _nb_sin(freq, n_samples, volume, sr):
        out = np.empty(n_samples, dtype=np.float64); two_pi = 2.0 * math.pi
        for i in range(n_samples): out[i] = 32767.0 * volume * math.sin(two_pi * freq * i / sr)
        return out
    @njit(cache=True, nogil=True)
    ...
    def _nb_mix(a, b):
        ...
        return out
    def _nb_clip_i16(samples): return np.clip(samples, -32768, 32767).astype(np.int16)

def _sin(freq, duration, volume=0.5, sr=44100): return _nb_sin(freq, int(sr * duration), volume, sr)
def _sweep(start_freq, end_freq, duration, volume=0.5, sr=44100): return _nb_sweep(start_freq, end_freq, int(sr * duration), volume, sr)
def _env(samples, attack=0.01, release=0.02, sr=44100): return _nb_env(samples, attack, release, sr)
...
class SoundManager:
    ...
    def play(self, name):
        ...
        if s: s.stop(); s.play()
    def cleanup(self):
        try: shutil.rmtree(self.tmpdir, ignore_errors=True); log("Sound temp dir cleaned up", "SOUND")
        except Exception as e: log(f"Sound cleanup error: {e}", "SOUND")
...
if HAS_NUMBA:
    ...
    def _fix_stride_nb(raw, w, h, bpl): return raw[:, :w * 3].reshape(h, w, 3)
...
class ChessBoardWidget(QWidget):
    ...
    def _get_anim_state(self):
        ...
        return {'from': self.anim_from, 'to': self.anim_to, 'piece_obj': self.anim_piece_obj, 'progress': t_eased}
    def paintEvent(self, e):
        chk = self.engine.check_squares()
        img = self.render_frame(self.engine.board, self.engine.last_move, self.selected,
                                self.legal_targets, check_squares=chk, anim_state=self._get_anim_state(),
                                theme=self.current_theme)
        pix = QPixmap.fromImage(img); painter = QPainter(self); painter.drawPixmap(0, 0, pix); painter.end()
    @staticmethod
    def render_frame(board, last_move=None, selected=None, legal_targets=None,
        ...
        img.fill(Qt.transparent); p = QPainter(img); p.setRenderHint(QPainter.Antialiasing)
        ...
        for sq in chess.SQUARES:
            r, c = 7 - chess.square_rank(sq), chess.square_file(sq); x, y = c * sz, r * sz
            is_light = (r + c) % 2 == 0; color = theme.light_sq if is_light else theme.dark_sq
        ...
        if anim_state and anim_state.get('captured', '.') != '.':
            ...
            if cap_piece is None:
                ...
                pt_map = {'K': chess.KING, 'Q': chess.QUEEN, 'R': chess.ROOK, 'B': chess.BISHOP, 'N': chess.KNIGHT, 'P': chess.PAWN}
                ...
                if pt:
                    ...
                    p.setOpacity(fade / 255.0); ChessBoardWidget._draw_piece(p, cap_piece, tr, tc_, sz, font_piece); p.setOpacity(1.0)
        if anim_state:
            fr, fc_ = anim_state['from']; tr, tc_ = anim_state['to']; t = anim_state['progress']
            ...
            if anim_piece_obj:
                ...
                sy = ir * sz + sz * 0.82
                p.drawEllipse(QRectF(ic * sz + (sz * scale - sz * 0.65) / 2, sy, sz * 0.65, sz * 0.12))
                w, h = sz * scale, sz * scale; y_lift = ir * sz - (sz * lift)
                ChessBoardWidget._draw_piece_at(p, anim_piece_obj, y_lift / sz, ic, sz, w, h, font_piece)
        p.setFont(font_coord); coord_margin = max(3, int(sz * 0.04)); coord_sz = max(12, sz // 5)
        for c in range(8):
            is_light = (7 + c) % 2 == 0; col = theme.dark_sq if is_light else theme.light_sq; p.setPen(col)
            p.drawText(QRect(c * sz + sz - coord_sz - coord_margin, 7 * sz + coord_margin, coord_sz, coord_sz), Qt.AlignCenter, FILES_STR[c])
        for r in range(8):
            is_light = r % 2 == 0; col = theme.dark_sq if is_light else theme.light_sq; p.setPen(col)
            p.drawText(QRect(coord_margin, r * sz + coord_margin, coord_sz, coord_sz), Qt.AlignCenter, RANKS_STR[r])
        if text_overlay:
    ...
    def render_card(text, bg="#1a1a2e", fg="#e0e0e0", w=544, h=544,
                    width=None, height=None, font_size=36, sub_text="", bg_color=None, fg_color=None):
        bg_val = bg if bg != "#1a1a2e" else (bg_color or bg)
        fg_val = fg if fg != "#e0e0e0" else (fg_color or fg)
        w_val = width if width is not None else w; h_val = height if height is not None else h
        img = QImage(w_val, h_val, QImage.Format_ARGB32_Premultiplied); img.fill(QColor(bg_val))
        p = QPainter(img); p.setRenderHint(QPainter.Antialiasing); p.setPen(QColor(fg_val))
        p.setFont(QFont("Sans", font_size, QFont.Bold))
        p.drawText(QRect(0, 0, w_val, h_val), Qt.AlignCenter, text)
        if sub_text:
            p.setFont(QFont("Sans", max(10, font_size // 2))); p.setPen(QColor(fg_val).lighter(140))
            p.drawText(QRect(0, h_val * 3 // 5, w_val, h_val // 4), Qt.AlignCenter, sub_text)
        p.end(); return img
    @staticmethod
    ...
    def qimage_to_np(img):
        img2 = img.convertToFormat(QImage.Format_RGB888); ptr = img2.constBits()
    ...
    def qimage_to_np_batch(images, use_gpu=False):
        ...
        arrays = [ChessBoardWidget.qimage_to_np(im) for im in images]; stack = np.stack(arrays)
        if use_gpu and HAS_CUPY: import cupy as _cp; return _cp.asarray(stack)
        return stack
    def mousePressEvent(self, e):
        ...
        sq = self.engine.rc_to_sq(r, c); piece = self.engine.board.piece_at(sq)
        if self.selected:
            sr, sc = self.selected
            if (r, c) in self.legal_targets:
                ...
                if info:
                    ...
                    self.snd.play(sfx)
                    if self.anim_speed > 0: self.start_animation(sr, sc, r, c, info['piece_obj'], info['captured'], info['notation'])
                    else: self.move_made.emit(info['notation'])
            self.selected = None; self.legal_targets = []
        else:
...
def load_openings(filepath):
    ...
    for i in range(0, len(rows), 5000): yield _process_opening_rows(rows[i:i + 5000])
...
def _parse_sqlite(filepath):
    ...
    table_name = tables[0][0]; cursor = conn.execute(f'SELECT * FROM "{table_name}"')
    col_names = [desc[0] for desc in cursor.description]
    rows = [dict(zip(col_names, row)) for row in cursor.fetchall()]; conn.close(); return rows
...
def _process_opening_rows(rows):
    ...
    for row in rows:
        ...
        elif img_val is None: img_raw = ''
        else: img_raw = str(img_val)
        uci_val = row.get('uci', '')
        if isinstance(uci_val, list):
            uci_moves = []
            for item in uci_val:
                if isinstance(item, str): uci_moves.extend(item.replace(',', ' ').split())
        ...
        openings.append({'volume': str(row.get('eco-volume', '')), 'eco': eco, 'name': name,
...
class DataProvider:
    ...
    def _parquet_columns(path):
        try:
            if HAS_PYARROW:
                import pyarrow.parquet as pq; return [f.name for f in pq.read_schema(path)]
        except Exception: pass
        try:
            if HAS_PANDAS:
                import pandas as pd; return list(pd.read_parquet(path, columns=None).head(0).columns)
        except Exception: pass
        return []
        ...
        try:
            if HAS_PYARROW:
                ...
                for batch in pf.iter_batches(batch_size=scan_batch):
                    df = batch.to_pandas()
                    if id_set is not None: df = df[df['id'].isin(id_set)]; id_set -= set(df['id'].tolist())
                ...
                for rec in df.to_dict('records'): results.append(self._deserialize_record(rec))
                del df
        except Exception as e: log(f"Error reading {db_type} records: {e}", "DATA")
        return results
        ...
        try:
            if HAS_PYARROW:
                ...
                for rec in df.to_dict('records'):
                    ...
                    if len(buf) >= chunk_size: yield buf; buf = []
                del df
        except Exception as e: log(f"Error scanning {db_type}: {e}", "DATA")
        if buf: yield buf

    def _make_record(self, db_type, item, next_id):
        return {'id': next_id, 'name': str(item.get('name', '')),
                'eco': str(item.get('eco', '')), 'img_raw': str(item.get('img_raw', '')),
                'pgn': str(item.get('pgn', '')), 'uci_moves': item.get('uci_moves', []),
    ...
    def stream_import(self, db_type, chunk_generator):
        ...
        if HAS_PYARROW: yield from self._stream_import_pyarrow(db_type, chunk_generator, cols, path, self._next_id[db_type])
        elif HAS_PANDAS: yield from self._stream_import_pandas(db_type, chunk_generator, cols, path, self._next_id[db_type])
        else: raise ImportError("Need pandas or pyarrow for parquet I/O")
    ...
    def _append_records_to_parquet(self, db_type, records, cols):
        ...
        new_df = self._records_to_dataframe(save_records, cols); path = self._cache_path(db_type)
    ...
    def insert_batch(self, db_type, items):
        ...
        if HAS_PANDAS:
            ...
            else: self._slim[db_type] = new_slim
    ...
    def reload(self, db_type): self._load_slim(db_type); self._dirty[db_type] = False
    ...
    def get_count(self, db_type, filter_text=""):
        ...
        if filter_text:
            ...
            if 'display_title' in df.columns: mask |= df['display_title'].str.lower().str.contains(ft, na=False)
            if 'eco' in df.columns: mask |= df['eco'].str.lower().str.contains(ft, na=False)
            return int(mask.sum())
        return len(df)
    ...
    def get_page(self, db_type, page, page_size, filter_text=""):
        ...
        page_df = filtered.iloc[start:end]; results = []
...
class DataLoadWorker(QThread):
    data_ready = Signal(str, int); load_error = Signal(str, str)
    ...
    def run(self):
        try:
            ...
            for f in files:
                ...
                try:
                    chunk_gen = load_openings(str(f))
                    for chunk_count, running_total in db.stream_import(self.db_type, chunk_gen):
                        if self._abort: break
                        total_count = running_total
                        if chunk_count % 100_000 < 60_000: log(f"  {self.db_type}: {total_count:,} rows (+{chunk_count})", "DATA")
                except Exception as e:
                    log(f"Error loading {f.name}: {e}", "DATA"); import traceback; traceback.print_exc()
            ...
            log(f"Fatal load error ({self.db_type}): {e}", "DATA"); import traceback; traceback.print_exc()
            self.load_error.emit(self.db_type, str(e)); self.data_ready.emit(self.db_type, 0)
    def _get_files(self):
...
# ═══════════════════════════════════════════════════════════════════════════════
#  EXPORT (same as puzzle app — reused for opening video export)
# ═══════════════════════════════════════════════════════════════════════════════
...
if HAS_NUMBA:
    ...
    def _normalize_frames_nb(frame_ptrs, target_h, target_w):
        n = frame_ptrs.shape[0]; out = np.empty((n, target_h, target_w, 3), dtype=np.uint8)
...
if HAS_CUPY:
    ...
    def _gpu_vignette(frames_gpu, strength=0.25):
        ...
        dist = _cp_export.sqrt(xx ** 2 + yy ** 2); vignette = 1.0 - strength * _cp_export.clip(dist / 1.414, 0, 1)
        vignette = vignette[_cp_export.newaxis, :, :, _cp_export.newaxis]
        return _cp_export.clip(frames_gpu.astype(_cp_export.float32) * vignette, 0, 255).astype(_cp_export.uint8)
    def _gpu_color_grade(frames_gpu, contrast=1.02, brightness=0.0, saturation=1.05):
        f = frames_gpu.astype(_cp_export.float32); f = _cp_export.clip(f * contrast + brightness, 0, 255)
        if saturation != 1.0:
...
def _render_puzzle_frames(puzzle, cfg, abort_check=None):
    ...
    if cfg.title_enabled and title_text:
        ...
        for _ in range(n_frames):
            tasks.append(('card', {'text': title_text, 'bg': cfg.title_bg, 'fg': cfg.title_fg,
                                   'w': tw if needs_composite else bpx, 'h': th if needs_composite else bpx,
    ...
    for move_str in puzzle["moves"]:
        ...
        board_before_move = eng.board.copy(); move = None
        try:
    ...
    if cfg.end_enabled and cfg.end_text:
        ...
        for _ in range(n_frames):
            tasks.append(('card', {'text': cfg.end_text, 'bg': cfg.end_bg, 'fg': cfg.end_fg,
                                   'w': tw if needs_composite else bpx, 'h': th if needs_composite else bpx,
    ...
    def render_task(idx_task):
        ...
        if t_type == 'card':
            img = ChessBoardWidget.render_card(**t_kwargs); np_arr = ChessBoardWidget.qimage_to_np(img)
    ...
    with ThreadPoolExecutor(max_workers=cfg.max_workers) as executor:
        ...
        for future in as_completed(futures):
            if abort_check and abort_check(): executor.shutdown(wait=False, cancel_futures=True); return None
            try:
                idx, np_arr = future.result(); frames[idx] = np_arr
            except Exception as e: log(f"Frame render error: {e}", "EXPORT")
...
def _post_process_frames(frames, cfg, abort_check=None):
    ...
    if needs_resize:
        ...
        frames_np = _normalize_frames_nb(frame_ptrs, target_h, target_w); frames = [frames_np[i] for i in range(len(frames))]
    use_gpu = (HAS_CUPY and cfg.gpu_post_process and (cfg.gpu_vignette > 0 or cfg.gpu_contrast != 1.0 or cfg.gpu_saturation != 1.0))
    if use_gpu:
        try: frames = _gpu_post_process(frames, cfg)
        except Exception as e: log(f"CuPy GPU post-processing failed ({e}), skipping", "EXPORT")
    return frames
...
def _gpu_post_process(frames, cfg):
    ...
    for start in range(0, n, chunk):
        ...
        del gpu, stack_np, cpu; _cp_export.get_default_memory_pool().free_all_blocks()
    return result

def _write_mp4(filepath, frames, fps, cfg=None):
    if not HAS_NUMPY or not HAS_IMAGEIO: return False, "ERROR: Missing numpy or imageio"
    use_ffmpeg = HAS_FFMPEG and (cfg.use_ffmpeg if cfg else True)
    if use_ffmpeg and cfg and len(frames) > 0:
        h, w = frames[0].shape[:2]
        try:
            tmp_dir = tempfile.mkdtemp(prefix="chess_frames_"); write_frames_to_disk(frames, tmp_dir)
            crf = cfg.ffmpeg_crf if cfg else 20; preset = cfg.ffmpeg_preset if cfg else "medium"
            ok, msg = write_mp4_ffmpeg(tmp_dir, filepath, fps, w, h, crf, preset)
            try: shutil.rmtree(tmp_dir)
            except OSError: pass
            if ok: return True, msg
        except Exception as e: log(f"FFmpeg encode error ({e}), falling back", "EXPORT")
    try:
        import imageio.v3 as iio; iio.imwrite(filepath, frames, fps=fps); return True, f"Saved: {filepath}"
    except AttributeError:
        try:
            import imageio; imageio.mimwrite(filepath, frames, fps=fps); return True, f"Saved: {filepath}"
        except Exception as e2: return False, f"Error: {e2}"
...
def _write_gif(filepath, frames, fps, cfg=None):
    if HAS_FFMPEG and len(frames) > 0:
        h, w = frames[0].shape[:2]
        try:
            tmp_dir = tempfile.mkdtemp(prefix="chess_gif_"); write_frames_to_disk(frames, tmp_dir)
...
def composite_frame(board_np, target_w, target_h, bg, title_overlay="", subtitle_overlay=""):
    ...
    bh, bw = board_np.shape[:2]; x = (target_w - bw) // 2; y = (target_h - bh) // 2
    ...
    if title_overlay or subtitle_overlay: canvas = _render_overlays(canvas, title_overlay, subtitle_overlay)
    return canvas

def composite_card_frame(card_np, target_w, target_h, bg):
    canvas = np.full((target_h, target_w, 3), bg, dtype=np.uint8)
    ch, cw = card_np.shape[:2]; x = (target_w - cw) // 2; y = (target_h - ch) // 2
    y1, y2 = max(0, y), min(target_h, y + ch); x1, x2 = max(0, x), min(target_w, x + cw)
    by1, by2 = y1 - y, y2 - y; bx1, bx2 = x1 - x, x2 - x
    canvas[y1:y2, x1:x2] = card_np[by1:by2, bx1:bx2]; return canvas
...
def _render_overlays(canvas, title_text, subtitle_text):
    ...
    if title_text:
        ...
        margin = int(h * 0.02); p.drawText(QRect(margin, margin, w - 2 * margin, font_size + 10), Qt.AlignLeft | Qt.AlignTop, title_text)
    if subtitle_text:
    ...
    p.end(); ptr = img.constBits()
    if hasattr(ptr, 'setsize'): ptr.setsize(img.sizeInBytes())
    raw = np.frombuffer(ptr, dtype=np.uint8).reshape(h, w * 3).copy(); return raw.reshape(h, w, 3)

def write_frames_to_disk(frames, output_dir):
    os.makedirs(output_dir, exist_ok=True)
    if HAS_IMAGEIO:
        import imageio.v3 as iio
        for i, frame in enumerate(frames): iio.imwrite(os.path.join(output_dir, f"frame_{i:06d}.png"), frame)
    else:
        for i, frame in enumerate(frames):
            h, w = frame.shape[:2]; img = QImage(frame.copy().data, w, h, w * 3, QImage.Format_RGB888).copy()
...
def write_mp4_ffmpeg(frame_dir, output_path, fps, width, height, crf=20, preset="medium"):
    if not HAS_FFMPEG: return False, "FFmpeg not found"
    cmd = ["ffmpeg", "-y", "-framerate", str(fps), "-i", os.path.join(frame_dir, "frame_%06d.png"),
           "-c:v", "libx264", "-pix_fmt", "yuv420p", "-crf", str(crf), "-preset", preset, "-movflags", "+faststart", output_path]
    try:
...
def write_gif_ffmpeg(frame_dir, output_path, fps, width, height):
    if not HAS_FFMPEG: return False, "FFmpeg not found"
    cmd = ["ffmpeg", "-y", "-framerate", str(fps), "-i", os.path.join(frame_dir, "frame_%06d.png"),
           "-filter_complex", f"[0:v] fps={fps},split [a][b];[a] palettegen [p];[b][p] paletteuse", output_path]
    try:
        result = subprocess.run(cmd, capture_output=True, text=True, timeout=600)
...
def mix_audio_ffmpeg(video_path, audio_path, output_path, volume=0.25):
    ...
    try:
        result = subprocess.run(cmd, capture_output=True, text=True, timeout=600)
        if result.returncode == 0: return True, "Audio mixed"
...
class BatchExportWorker(QThread):
    ...
    def _unique_path(self, base_name, ext=None):
        ...
        safe = sanitize_filename(base_name); path = os.path.join(self.output_dir, safe + ext)
    ...
    def run(self):
        ...
        for i, puzzle in enumerate(self.puzzles):
            if self._abort: break
            name = puzzle.get('name', f'opening_{i+1}')
            ...
            try:
                ...
                if self.config.export_gif: ok, msg = _write_gif(filepath, frames, self.config.gif_fps, self.config)
                else: ok, msg = _write_mp4(filepath, frames, self.config.fps, self.config)
                if ok:
                ...
                else: errors += 1; self.puzzle_error.emit(i, msg)
            except Exception as e: errors += 1; self.puzzle_error.emit(i, f"Error: {e}")
...
class ExportWorker(QThread):
    ...
    def abort(self): self._abort = True
    def run(self):
        if not HAS_NUMPY or not HAS_IMAGEIO: self.finished.emit("ERROR: Missing numpy or imageio"); return
        ...
        if self.config.export_gif: ok, msg = _write_gif(self.file_path, frames, self.config.gif_fps, self.config)
        else: ok, msg = _write_mp4(self.file_path, frames, self.config.fps, self.config)
...
class MainWindow(QWidget):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("♚ Chess Openings App")
        log("Initializing Chess Openings App...", "APP")
        ...
        self.openings_loaded = False; self._active_workers = []
        self._op_page = 0; self._op_checked = set()
        self._current_opening = None; self.opening_step_idx = 0
        ...
        self._build_openings_tab(); self._build_settings_tab()
        self.snd.play("start"); log("App initialization complete", "APP")
        QTimer.singleShot(50, lambda: self._start_data_load("openings"))
    ...
    def _change_theme(self, name):
        if name in THEMES:
            ...
            if hasattr(self, 'settings_theme_cb'):
                self.settings_theme_cb.blockSignals(True); self.settings_theme_cb.setCurrentText(name); self.settings_theme_cb.blockSignals(False)
    ...
    def _update_anim_speed(self, raw_val):
        ...
        if hasattr(self, 'settings_anim_slider'):
            ...
            self.settings_anim_slider.blockSignals(True); self.settings_anim_slider.setValue(raw_val); self.settings_anim_slider.blockSignals(False)
    ...
    def _start_data_load(self, db_type, single_file=None):
        ...
        if single_file: worker = DataLoadWorker(db_type, single_file=single_file)
        else: worker = DataLoadWorker(db_type, directory=str(Path(DATA_DIR) / db_type))
    ...
    def _on_data_ready(self, db_type, total_count):
        self.db.reload(db_type); self.openings_loaded = True
        self._op_checked.clear(); self._op_page = 0
    ...
    def _build_openings_tab(self):
        ...
        # ── 1. Database ──────────────────────────────────────────────────
        db_group = QGroupBox("📂 Openings Database"); db_layout = QVBoxLayout(db_group)
        path_row = QHBoxLayout()
        self.opening_db_path = QLineEdit()
        self.opening_db_path.setPlaceholderText("Database file path (.csv .parquet .pq .duckdb .db .sqlite)…")
        path_row.addWidget(self.opening_db_path, 1)
        btn_load_db = QPushButton("Load"); btn_load_db.clicked.connect(self.load_openings_db)
        ...
        db_layout.addWidget(self.opening_db_status); l.addWidget(db_group)

        # ── 2. Filter & Selection ────────────────────────────────────────
        filter_group = QGroupBox("🔍 Filter & Selection"); fl = QVBoxLayout(filter_group)
        filter_row = QHBoxLayout()
        self.opening_filter = QLineEdit(); self.opening_filter.setPlaceholderText("Filter openings…")
        filter_row.addWidget(self.opening_filter, 1); fl.addLayout(filter_row)
        self._op_filter_timer = QTimer(); self._op_filter_timer.setSingleShot(True)
        self._op_filter_timer.setInterval(300); self._op_filter_timer.timeout.connect(self._apply_opening_filter)
        self.opening_filter.textChanged.connect(lambda: self._op_filter_timer.start())
        sel_row = QHBoxLayout()
        btn_all = QPushButton("All"); btn_all.setFixedWidth(55); btn_all.clicked.connect(self._opening_select_all)
        btn_none = QPushButton("None"); btn_none.setFixedWidth(55); btn_none.clicked.connect(self._opening_select_none)
        btn_inv = QPushButton("Invert"); btn_inv.setFixedWidth(60); btn_inv.clicked.connect(self._opening_select_invert)
        ...
        self.opening_range_from = QSpinBox(); self.opening_range_from.setRange(1, 999999); self.opening_range_from.setPrefix("#")
        self.opening_range_to = QSpinBox(); self.opening_range_to.setRange(1, 999999); self.opening_range_to.setPrefix("#")
        btn_range = QPushButton("Apply Range"); btn_range.clicked.connect(self._opening_select_range)
        ...
        self.opening_sel_label.setAlignment(Qt.AlignRight); fl.addWidget(self.opening_sel_label)
        l.addWidget(filter_group)

        # ── 3. Openings List ─────────────────────────────────────────────
        list_group = QGroupBox("📚 Openings List"); ll = QVBoxLayout(list_group)
        self.opening_list = CheckableListWidget(); self.opening_list.setMaximumHeight(130)
        ...
        nav = QHBoxLayout()
        self.btn_op_prev = QPushButton("◀"); self.btn_op_prev.clicked.connect(self._op_prev_page)
        self.op_page_lbl = QLabel("Page 0 / 0"); self.op_page_lbl.setAlignment(Qt.AlignCenter)
        self.btn_op_next = QPushButton("▶"); self.btn_op_next.clicked.connect(self._op_next_page)
        nav.addWidget(self.btn_op_prev); nav.addWidget(self.op_page_lbl, 1); nav.addWidget(self.btn_op_next)
        ...
        btn_op_jump = QPushButton("Go"); btn_op_jump.setFixedWidth(35); btn_op_jump.clicked.connect(self._op_jump_page)
        nav.addWidget(self.op_jump_spin); nav.addWidget(btn_op_jump); ll.addLayout(nav)
        l.addWidget(list_group)

        # ── 4. Opening Detail ────────────────────────────────────────────
        detail_group = QGroupBox("♟ Opening Detail"); dl = QVBoxLayout(detail_group)
        self.opening_img_lbl = QLabel("Opening Image")
        self.opening_img_lbl.setFixedSize(220, 220); self.opening_img_lbl.setAlignment(Qt.AlignCenter)
        self.opening_img_lbl.setStyleSheet("background-color: #2b2b2b; border: 1px solid #555; border-radius: 4px;")
        dl.addWidget(self.opening_img_lbl, alignment=Qt.AlignCenter)
        step_layout = QHBoxLayout(); step_layout.addStretch()
        ...
        step_layout.addStretch(); dl.addLayout(step_layout)
        ...
        # ── 5. Export ────────────────────────────────────────────────────
        export_group = QGroupBox("🎬 Export Openings"); el = QVBoxLayout(export_group)
        gif_row = QHBoxLayout()
        self.opening_exp_gif = QCheckBox("Export as GIF"); gif_row.addWidget(self.opening_exp_gif)
        self.opening_exp_gif_fps = QSpinBox(); self.opening_exp_gif_fps.setRange(5, 30)
        self.opening_exp_gif_fps.setValue(12); self.opening_exp_gif_fps.setEnabled(False)
        self.opening_exp_gif.toggled.connect(self.opening_exp_gif_fps.setEnabled)
        gif_row.addWidget(QLabel("GIF FPS:")); gif_row.addWidget(self.opening_exp_gif_fps)
        el.addLayout(gif_row)
        batch_row = QHBoxLayout()
        btn_exp_sel = QPushButton("Export Selected"); btn_exp_sel.clicked.connect(self._export_openings_batch)
        btn_exp_all = QPushButton("Export All"); btn_exp_all.clicked.connect(self._export_all_openings_batch)
        batch_row.addWidget(btn_exp_sel); batch_row.addWidget(btn_exp_all)
        el.addLayout(batch_row)
        self.opening_progress = QProgressBar(); self.opening_progress.setRange(0, 100)
        self.opening_progress.setValue(0); el.addWidget(self.opening_progress)
        self.opening_status = QLabel(""); self.opening_status.setWordWrap(True)
        el.addWidget(self.opening_status)
        l.addWidget(export_group)
        l.addStretch(); scroll.setWidget(container)
    ...
    def _populate_opening_page(self):
        ...
        self._update_opening_nav(); self._update_opening_sel_label()

    def _update_opening_nav(self):
        total = self._op_total_items(); pc = max(1, math.ceil(total / self._PAGE_SIZE))
        ...
        self.op_jump_spin.setRange(1, pc); self.op_jump_spin.setValue(self._op_page + 1)
    ...
    def _update_opening_sel_label(self, _item=None):
        cnt = len(self._op_checked); total = self.db.get_count('openings')
    ...
    def _on_opening_item_changed(self, item):
        ...
        if item.checkState() == Qt.Checked: self._op_checked.add(di)
        else: self._op_checked.discard(di)
        self._update_opening_sel_label()

    def _apply_opening_filter(self):
        self._op_page = 0; self._populate_opening_page()

    def _opening_select_all(self):
        self._op_checked = set(self.db.get_ids_by_filter('openings', self.opening_filter.text().strip()))
        self._populate_opening_page()

    def _opening_select_none(self):
        self._op_checked.clear(); self._populate_opening_page()

    def _opening_select_invert(self):
        all_ids = set(self.db.get_ids_by_filter('openings', self.opening_filter.text().strip()))
        self._op_checked = all_ids - self._op_checked; self._populate_opening_page()

    def _opening_select_range(self):
        start = self.opening_range_from.value(); end = self.opening_range_to.value()
    ...
    def _op_prev_page(self):
        if self._op_page > 0: self._op_page -= 1; self._populate_opening_page()

    def _op_next_page(self):
        if self._op_page < self._op_page_count() - 1: self._op_page += 1; self._populate_opening_page()

    def _op_jump_page(self):
        page = self.op_jump_spin.value() - 1
        if 0 <= page < self._op_page_count(): self._op_page = page; self._populate_opening_page()
    ...
    def select_opening(self, row):
        ...
        if not items: return
        data = items[0]; self._current_opening = data; self.opening_step_idx = 0
        self.engine.load_fen(self._opening_fen(data))
        img_raw = data.get('img_raw', ''); img = parse_opening_image(img_raw) if img_raw else None
        if img and not img.isNull():
            pixmap = QPixmap.fromImage(img)
            self.opening_img_lbl.setPixmap(pixmap.scaled(210, 210, Qt.KeepAspectRatio, Qt.SmoothTransformation))
    ...
    def update_opening_display(self):
        if not hasattr(self, '_current_opening') or not self._current_opening: return
        data = self._current_opening; text = ""
    ...
    def opening_prev(self):
        ...
        if self.opening_step_idx > 0:
            ...
            self.engine.load_fen(self._opening_fen(self._current_opening))
            for uci in self._current_opening.get('uci_moves', [])[:self.opening_step_idx]:
    ...
    def opening_next(self):
        ...
        if self.opening_step_idx < len(uci_moves):
            ...
            self.engine.make_move_uci(uci); self.opening_step_idx += 1
            self.board_widget.selected = None; self.board_widget.legal_targets = []
            self.board_widget.update(); self.update_opening_display(); self.snd.play("move")
    ...
    def opening_end(self):
        ...
        uci_moves = self._current_opening.get('uci_moves', [])
        self.engine.load_fen(self._opening_fen(self._current_opening))
        for uci in uci_moves: self.engine.make_move_uci(uci)
    ...
    def _opening_to_puzzle(self, data):
        ...
        return {'name': name, 'display_title': data.get('display_title', name),
                'fen': self._opening_fen(data), 'moves': data.get('uci_moves', []),
                'desc': data.get('pgn', ''), 'difficulty': 0.5}
    ...
    def _export_all_openings_batch(self):
        ...
        if total == 0: self.opening_status.setText("No openings loaded."); return
        all_ids = self.db.get_ids_by_filter('openings')
    ...
    def _build_export_config(self, puzzle_name=""):
        ...
        cfg.end_text = "Solved!"; cfg.fps = 30; cfg.max_workers = 4
        cfg.theme_name = self.theme_cb.currentText(); cfg.output_dir = ""
        cfg.gpu_post_process = HAS_CUPY; cfg.gpu_vignette = 0.25
        cfg.gpu_contrast = 1.02; cfg.gpu_saturation = 1.05
        cfg.apply_preset("Board Only (544×544)")
        return cfg

    def _ensure_output_dir(self):
        d = ""
        if hasattr(self, 'settings_outdir'): d = self.settings_outdir.text().strip()
        if not d: d = str(Path(DATA_DIR) / "exports")
    ...
    def _on_opening_batch_done(self, idx, filepath): pass
    ...
    def _on_opening_batch_all_done(self, exported, errors, out_dir):
        ...
        self.opening_status.setText(f"Batch done: {exported} exported, {errors} errors → {out_dir}")
    ...
    def load_openings_db(self):
        ...
        if not path: self.opening_db_status.setText("Enter a database path above first."); return
        if not os.path.exists(path): self.opening_db_status.setText(f"File not found: {path}"); return
        self.opening_db_status.setText("Loading…"); self._start_data_load("openings", single_file=path)
    ...
    def _build_settings_tab(self):
        ...
        db_info_layout.addWidget(QLabel(f"Openings cache: {self.db._cache_path('openings')}"))
        l.addWidget(db_info_group); l.addStretch(); scroll.setWidget(container)
        self.tabs.addTab(scroll, "⚙️ Settings")


def main():
    log("Launching Chess Openings App…", "APP")
    app = QApplication(sys.argv); app.setStyle("Fusion")
    shell = QMainWindow(); shell.setWindowTitle("♚ Chess Openings App")
