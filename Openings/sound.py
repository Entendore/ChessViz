"""
sound.py — Audio primitives (numba-accelerated when available) and SoundManager.
"""

import math, os, tempfile, shutil, wave
import numpy as np
from PySide6.QtMultimedia import QSoundEffect
from PySide6.QtCore import QUrl
from config import HAS_NUMBA, log

# ═══════════════════════════════════════════════════════════════════════════════
#  LOW-LEVEL AUDIO PRIMITIVES (numba JIT or pure-numpy fallback)
# ═══════════════════════════════════════════════════════════════════════════════
...
class SoundManager:
    ...
    def _gen_all(self):
        ...
        cm = np.concatenate([_sin(800, 0.15, 0.5), _sin(600, 0.15, 0.5),
                             _sin(400, 0.25, 0.5)])
    ...
    def _load_all(self):
        for n in ("move", "capture", "check", "checkmate", "castle",
                  "error", "promote", "start"):
            e = QSoundEffect()
    ...
    def cleanup(self):
        try:
            ...
            log("Sound temp dir cleaned up", "SOUND")
