...
import os, wave, math, tempfile, shutil
import numpy as np
...
class SoundManager:
    ...
    def _gen_all(self):
        ...
        self._wav(os.path.join(d, "capture.wav"),
                  _env(_mix(_sin(300, 0.10, 0.5), _sin(600, 0.08, 0.3)),
                       0.005, 0.04))

        self._wav(os.path.join(d, "check.wav"),
                  _env(_mix(_sin(1000, 0.12, 0.5), _sin(1250, 0.10, 0.3)),
                       0.005, 0.04))
    ...
    # IMPROVEMENT: clean up temp WAV files on exit
    def cleanup(self):
        """Remove temporary WAV directory."""
        try:
