...
Supports streaming import, chunked reads, and slim (metadata-only) loading.
"""

import os, json, shutil
from config import (
    DATA_DIR, DB_OPENINGS_PATH, HAS_PANDAS, HAS_PYARROW, HAS_DUCKDB, log
)
...
class DataProvider:
    ...
    def stream_import(self, db_type, chunk_generator):
        ...
        if HAS_PYARROW:
            yield from self._stream_import_pyarrow(
                db_type, chunk_generator, cols, path, self._next_id[db_type])
        elif HAS_PANDAS:
        ...
        try:
            for chunk in chunk_generator:
                ...
                for item in chunk:
                    ...
                    records.append(self._serialize_record_for_parquet(rec, cols))
                    next_id += 1
    ...
    def insert_batch(self, db_type, items):
        """Insert a batch of opening items into the cache."""
    ...
    def search_openings(self, query, limit=50):
        """Search openings by name/eco in the slim index."""
        import pandas as pd
