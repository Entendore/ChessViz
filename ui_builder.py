...
def build_ui(w):
    ...
    _build_pipeline_tab(w)  # NEW: dedicated pipeline tab
    _build_battle_tab(w)
...
# ════════════════════════════════════════════════════════════════════
#  Pipeline Tab — one-click PGN→MP4, AI→MP4, and Batch PGN
# ════════════════════════════════════════════════════════════════════
def _build_pipeline_tab(w):
    ...
    w.quick_quality_combo = QComboBox()
    for name, p in QUALITY_PRESETS.items():
        w.quick_quality_combo.addItem(p["label"], name)
    rec = get_recommended_preset()
    idx = list(QUALITY_PRESETS.keys()).index(rec)
    w.quick_quality_combo.setCurrentIndex(idx)
    qrow.addWidget(w.quick_quality_combo, stretch=1)
    pgl.addLayout(qrow)

    # Hold time
    hrow = QHBoxLayout()
    hrow.addWidget(QLabel("Hold per move:"))
    w.quick_hold_spin = QDoubleSpinBox()
    w.quick_hold_spin.setRange(0.3, 10.0); w.quick_hold_spin.setValue(1.5)
    w.quick_hold_spin.setSingleStep(0.1); w.quick_hold_spin.setSuffix(" s")
    hrow.addWidget(w.quick_hold_spin)
    pgl.addLayout(hrow)
    ...
    w.quick_pgn_btn.setMinimumHeight(40)
    w.quick_pgn_btn.setStyleSheet(
        "QPushButton{background:#2a6a2a;color:white;font-size:14px;font-weight:bold;"
        "border-radius:6px;padding:8px}"
        "QPushButton:hover{background:#3a8a3a}"
        "QPushButton:disabled{background:#444;color:#888}")
    w.quick_pgn_btn.clicked.connect(w._quick_pgn_to_mp4)
    pgl.addWidget(w.quick_pgn_btn)

    # Progress
    w.quick_progress_bar = QProgressBar(); w.quick_progress_bar.setValue(0)
    ...
    w.quick_battle_delay.setRange(10, 2000); w.quick_battle_delay.setValue(100)
    ...
    # AI quality
    aqrow = QHBoxLayout()
    aqrow.addWidget(QLabel("Quality:"))
    w.quick_ai_quality_combo = QComboBox()
    for name, p in QUALITY_PRESETS.items():
        w.quick_ai_quality_combo.addItem(p["label"], name)
    w.quick_ai_quality_combo.setCurrentIndex(idx)
    aqrow.addWidget(w.quick_ai_quality_combo, stretch=1)
    agl.addLayout(aqrow)

    # AI hold
    ahrow = QHBoxLayout()
    ahrow.addWidget(QLabel("Hold:"))
    w.quick_ai_hold_spin = QDoubleSpinBox()
    w.quick_ai_hold_spin.setRange(0.3, 10.0); w.quick_ai_hold_spin.setValue(1.5)
    w.quick_ai_hold_spin.setSingleStep(0.1); w.quick_ai_hold_spin.setSuffix(" s")
    ahrow.addWidget(w.quick_ai_hold_spin)
    agl.addLayout(ahrow)

    # Go button
    w.quick_ai_btn = QPushButton("🚀 AI Battle → MP4")
    w.quick_ai_btn.setMinimumHeight(40)
    w.quick_ai_btn.setStyleSheet(
        "QPushButton{background:#6a2a2a;color:white;font-size:14px;font-weight:bold;"
        "border-radius:6px;padding:8px}"
        "QPushButton:hover{background:#8a3a3a}"
        "QPushButton:disabled{background:#444;color:#888}")
    w.quick_ai_btn.clicked.connect(w._quick_ai_to_mp4)
    agl.addWidget(w.quick_ai_btn)

    # AI progress
    w.quick_ai_progress_bar = QProgressBar(); w.quick_ai_progress_bar.setValue(0)
    ...
    # Quality quick-select
    bqrow = QHBoxLayout()
    bqrow.addWidget(QLabel("Quality:"))
    w.batch_quality_combo = QComboBox()
    for name, p in QUALITY_PRESETS.items():
        w.batch_quality_combo.addItem(p["label"], name)
    # Inherit recommended index from earlier
    rec_idx = w.quick_quality_combo.currentIndex() 
    w.batch_quality_combo.setCurrentIndex(rec_idx)
    bqrow.addWidget(w.batch_quality_combo, stretch=1)
    bgl.addLayout(bqrow)

    # Hold time
    bhrow = QHBoxLayout()
    bhrow.addWidget(QLabel("Hold per move:"))
    w.batch_hold_spin = QDoubleSpinBox()
    w.batch_hold_spin.setRange(0.3, 10.0); w.batch_hold_spin.setValue(1.5)
    w.batch_hold_spin.setSingleStep(0.1); w.batch_hold_spin.setSuffix(" s")
    bhrow.addWidget(w.batch_hold_spin)
    bgl.addLayout(bhrow)
    ...
    w.batch_start_btn.setMinimumHeight(40)
    w.batch_start_btn.setStyleSheet(
        "QPushButton{background:#2a4a6a;color:white;font-size:14px;font-weight:bold;"
        "border-radius:6px;padding:8px}"
        "QPushButton:hover{background:#3a6a8a}"
        "QPushButton:disabled{background:#444;color:#888}")
    w.batch_start_btn.clicked.connect(w._start_batch_pgn_export)
    bgl.addWidget(w.batch_start_btn)

    # Progress
    w.batch_progress_bar = QProgressBar(); w.batch_progress_bar.setValue(0)
...
def _build_video_tab(w):
    ...
    # Performance & Quality
    perf_group = QGroupBox("⚡ Performance & Quality"); perf_l = QFormLayout(perf_group)
    ...
    ram = get_system_ram_gb(); gpu_name, gpu_vram = get_gpu_info()
    gpu_str = f"{gpu_name} ({gpu_vram:.1f} GB)" if gpu_vram > 0 else gpu_name
    w.sys_info_lbl = QLabel(f"RAM: {ram:.1f} GB  |  GPU: {gpu_str}")
    w.sys_info_lbl.setStyleSheet("color:#8ab;font-size:11px;padding:2px")
    w.sys_info_lbl.setWordWrap(True); perf_l.addRow("System:", w.sys_info_lbl)

    w.mem_estimate_lbl = QLabel("Est. memory: —")
    w.mem_estimate_lbl.setStyleSheet("color:#c90;font-size:11px;padding:2px")
    w.mem_estimate_lbl.setWordWrap(True); perf_l.addRow("Memory:", w.mem_estimate_lbl)
    ...
    # Board Appearance
    bg = QGroupBox("Board Appearance"); bl2 = QFormLayout(bg)
    ...
    bl2.addRow(w.flip_btn); vl.addWidget(bg)

    # Capture Settings
    eg = QGroupBox("Capture Settings"); el = QFormLayout(eg)
    w.fps_spin = QSpinBox(); w.fps_spin.setRange(1, 60)
    ...
    w.anim_spin = QDoubleSpinBox(); w.anim_spin.setRange(0.0, 3.0)
    w.anim_spin.setValue(0.3); w.anim_spin.setSingleStep(0.1); w.anim_spin.setSuffix(" s")
    ...
    w.hold_spin.setSingleStep(0.1); w.hold_spin.setSuffix(" s")
    w.hold_spin.valueChanged.connect(w._update_mem_estimate)
    el.addRow("Hold:", w.hold_spin); vl.addWidget(eg)
    ...
    w.export_fps_spin = QSpinBox(); w.export_fps_spin.setRange(1, 60)
...
def _build_settings_tab(w):
    ...
    w.sound_design_desc.setStyleSheet("color:#9ab;padding:4px;font-size:11px;background:#1e1e24;border-radius:4px")
    sgl.addWidget(w.sound_design_desc)

    nn = {"move":"♟ Move","capture":"⚔ Capture","check":"⚡ Check","checkmate":"🏁 Mate",
          "castle":"🏰 Castle","illegal":"🚫 Illegal","new_game":"🆕 New","promotion":"👑 Promo","ui_click":"🖱 Click"}
    w._snd_sliders = {}
    for st in SOUND_TYPES:
        r = QHBoxLayout(); r.addWidget(QLabel(nn.get(st, st)))
        s = QSlider(Qt.Horizontal); s.setRange(0, 100); s.setValue(100)
        l = QLabel("100%"); l.setFixedWidth(40)
        def cb(v, sr=s, ll=l, stt=st):
            w._on_snd_type_vol(stt, v / 100.0); ll.setText(f"{v}%")
        s.valueChanged.connect(cb); r.addWidget(s, stretch=1); r.addWidget(l)
        sgl.addLayout(r); w._snd_sliders[st] = s

    tbr = QHBoxLayout()
    for st, lb in [("move","♟"),("capture","⚔"),("check","⚡"),
                    ("checkmate","🏁"),("castle","🏰"),("new_game","🆕")]:
        b = QPushButton(lb); b.setFixedSize(36, 30)
        b.clicked.connect(lambda c, s=st: w._test_sound(s)); tbr.addWidget(b)
    sgl.addLayout(tbr); sgl.addStretch()
    ...
    w.piece_anim_chk = QCheckBox("Piece Movement"); w.piece_anim_chk.setChecked(True)
    w.piece_anim_chk.toggled.connect(w._on_piece_anim); agl.addWidget(w.piece_anim_chk)
    w.highlight_anim_chk = QCheckBox("Check/Flash Effects"); w.highlight_anim_chk.setChecked(True)
    w.highlight_anim_chk.toggled.connect(w._on_highlight_anim); agl.addWidget(w.highlight_anim_chk)
    w.eval_anim_chk = QCheckBox("Eval Bar Smooth"); w.eval_anim_chk.setChecked(True)
    w.eval_anim_chk.toggled.connect(w._on_eval_anim); agl.addWidget(w.eval_anim_chk)
    ...
    er = QHBoxLayout(); er.addWidget(QLabel("Easing:"))
    w.anim_ease_combo = QComboBox(); w.anim_ease_combo.addItems(ANIM_EASINGS)
    w.anim_ease_combo.setCurrentText("OutCubic")
    w.anim_ease_combo.currentTextChanged.connect(w._on_anim_ease)
    er.addWidget(w.anim_ease_combo, stretch=1); agl.addLayout(er)
    agl.addStretch()
