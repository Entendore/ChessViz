"""Chess Video Maker Pro — EvalBar, Promotion Widget, VideoRenderer"""
import math
import os
import chess
import logging
from PySide6.QtWidgets import QWidget, QSizePolicy, QHBoxLayout, QPushButton, QLabel
from PySide6.QtCore import Qt, QRectF, Signal, QPropertyAnimation, QEasingCurve, Property, QPointF
from PySide6.QtGui import (QPainter, QColor, QFont, QImage, QLinearGradient,
                            QRadialGradient, QPainterPath, QPen)

from constants import (GAME_NORMAL, GAME_CHECKMATE, GAME_STALEMATE,
                       GAME_DRAW, GAME_INSUFFICIENT)
from board_renderer import BoardRenderer

logger = logging.getLogger("ChessVideoMaker.Widgets")


# ════════════════════════════════════════════════════════════════════
#  EvalBarWidget — professional evaluation bar with game-state overlays
# ════════════════════════════════════════════════════════════════════
class EvalBarWidget(QWidget):
    BAR_WIDTH = 48; PADDING = 6; CORNER_RADIUS = 6
    TICK_MAJOR_W = 6; TICK_MINOR_W = 3; PILL_H = 22
    PILL_RADIUS = 11; PILL_PAD_X = 6
    CLR_OUTER_BG = QColor(18, 18, 22)
    CLR_BORDER = QColor(55, 55, 62)
    CLR_BLACK_TOP = QColor(62, 62, 70); CLR_BLACK_MID = QColor(48, 48, 55)
    CLR_BLACK_BOT = QColor(40, 40, 47)
    CLR_WHITE_BOT = QColor(248, 245, 238); CLR_WHITE_MID = QColor(240, 237, 228)
    ...
    def reset_game_state(self):
        self._game_state = GAME_NORMAL; self._game_result = ""
        self._game_detail = ""; self.update()
    ...
    def set_eval(self, cp):
        ...
        if self._game_state != GAME_NORMAL or abs(cp) > 9000 or abs(old) > 9000 or self._anim_dur == 0:
    ...
    def paintEvent(self, _):
        ...
        pad, cr = self.PADDING, self.CORNER_RADIUS
        bx, by, bw, bh = pad, pad, W - 2*pad, H - 2*pad

        # Outer frame
        p.setPen(Qt.NoPen); p.setBrush(QColor(0,0,0,50))
        p.drawRoundedRect(QRectF(1,2,W-1,H-1), cr+3, cr+3)
        p.setPen(QPen(self.CLR_BORDER, 1.2)); p.setBrush(self.CLR_OUTER_BG)
        ...
        if wh > 0:
            ...
            if is_mate or wh >= bh:
                ...
                path = QPainterPath(); path.moveTo(bx, wt)
                path.lineTo(bx+bw, wt); path.lineTo(bx+bw, by+bh-cr)
                path.quadTo(bx+bw, by+bh, bx+bw-cr, by+bh)
                path.lineTo(bx+cr, by+bh)
                path.quadTo(bx, by+bh, bx, by+bh-cr)
                path.lineTo(bx, wt); path.closeSubpath(); p.drawPath(path)
        ...
        # Tick marks
        p.setPen(Qt.NoPen); self._draw_ticks(p, bx, by, bw, bh)
        ...
        p.setPen(Qt.NoPen); p.setBrush(shine)
    ...
    def _draw_eval_pill(self, p, bx, by, bw, bh, wh, bdy, is_mate, W):
        ...
        if is_mate_val:
            ...
            pbg = QColor(255,255,255,215); pfg = QColor(35,32,28)
        else:
            pbg = QColor(22,22,30,220); pfg = QColor(238,234,226)
        pr = self.PILL_RADIUS; p.setPen(Qt.NoPen)
        p.setBrush(QColor(0,0,0,70)); p.drawRoundedRect(pill.adjusted(1,2,1,2), pr, pr)
        p.setBrush(pbg)
        p.setPen(QPen(QColor(255,255,255,30) if on_white else QColor(255,255,255,15), 0.8))
        ...
        p.setPen(Qt.NoPen); p.setBrush(glass)
        clip = QPainterPath(); clip.addRoundedRect(pill, pr, pr)
        p.save(); p.setClipPath(clip)
        p.drawRect(QRectF(pill.x(), pill.y(), pill.width(), pill.height()/2))
        p.restore()
        p.setPen(pfg); p.setFont(fnt); p.drawText(pill, Qt.AlignCenter, txt)

    def _draw_game_state_overlay(self, p, bx, by, bw, bh, W, H):
        if self._game_state == GAME_CHECKMATE:
            self._draw_checkmate_overlay(p, bx, by, bw, bh, W, H)
        elif self._game_state in (GAME_STALEMATE, GAME_DRAW, GAME_INSUFFICIENT):
            self._draw_draw_overlay(p, bx, by, bw, bh, W, H)

    def _draw_checkmate_overlay(self, p, bx, by, bw, bh, W, H):
        white_wins = (self._eval_cp > 0) or (self._game_result == "1-0")
        p.setPen(Qt.NoPen)
        tint = QColor(25,160,55,50) if white_wins else QColor(190,35,35,50)
        p.setBrush(tint); p.drawRoundedRect(QRectF(bx,by,bw,bh), self.CORNER_RADIUS, self.CORNER_RADIUS)
        center_y = by + bh/2.0; banner_h = min(80, max(50, int(bh*0.12)))
        banner_w = W - 2*self.PADDING
        banner = QRectF(self.PADDING, center_y - banner_h/2, banner_w, banner_h)
        bg = self.CLR_CHECKMATE_WHITE_WIN if white_wins else self.CLR_CHECKMATE_BLACK_WIN
        crown = "♔" if white_wins else "♚"
        p.setBrush(QColor(0,0,0,90)); p.drawRoundedRect(banner.adjusted(2,3,2,3), 8, 8)
        p.setBrush(bg); p.setPen(QPen(QColor(255,255,255,60), 1.0))
        ...
        glass.setColorAt(1.0, QColor(0,0,0,0))
        p.setPen(Qt.NoPen); p.setBrush(glass)
        clip = QPainterPath(); clip.addRoundedRect(banner, 8, 8)
        p.save(); p.setClipPath(clip)
        p.drawRect(QRectF(banner.x(), banner.y(), banner.width(), banner.height()/2))
        p.restore()
        crown_fnt = QFont("Segoe UI Symbol", max(10, int(banner_h*0.35))); p.setFont(crown_fnt)
        p.setPen(QColor(255,255,255,240))
        p.drawText(banner.adjusted(0,-2,0,-banner_h*0.35), Qt.AlignCenter, crown)
        cm_fnt = QFont("Segoe UI", max(7, int(banner_h*0.22)), QFont.Bold); p.setFont(cm_fnt)
        p.setPen(QColor(255,255,255,240))
        p.drawText(banner.adjusted(0, banner_h*0.08, 0, 0), Qt.AlignCenter, "CHECKMATE")
        res_fnt = QFont("Segoe UI", max(8, int(banner_h*0.25)), QFont.Bold); p.setFont(res_fnt)
        p.setPen(QColor(255,255,255,220))
        p.drawText(banner.adjusted(0, banner_h*0.35, 0, 0), Qt.AlignCenter,
                   self._game_result or ("1-0" if white_wins else "0-1"))
        glow = QColor(25,200,60,30) if white_wins else QColor(220,40,40,30)
        p.setPen(QPen(glow, 2.5)); p.setBrush(Qt.NoBrush)
        p.drawRoundedRect(QRectF(bx-1, by-1, bw+2, bh+2), self.CORNER_RADIUS+1, self.CORNER_RADIUS+1)

    def _draw_draw_overlay(self, p, bx, by, bw, bh, W, H):
        p.setPen(Qt.NoPen); p.setBrush(QColor(180,150,40,25))
        p.drawRoundedRect(QRectF(bx,by,bw,bh), self.CORNER_RADIUS, self.CORNER_RADIUS)
        center_y = by + bh/2.0
        ...
        bg = self.CLR_DRAW_AMBER if self._game_state == GAME_STALEMATE else self.CLR_DRAW_GRAY
        p.setBrush(bg); p.setPen(QPen(QColor(255,255,255,50), 1.0))
        p.drawRoundedRect(banner, 8, 8)
        glass = QLinearGradient(banner.topLeft(), banner.bottomLeft())
        ...
        p.drawRect(QRectF(banner.x(), banner.y(), banner.width(), banner.height()/2))
        p.restore()
        res_fnt = QFont("Segoe UI", max(10, int(banner_h*0.32)), QFont.Bold); p.setFont(res_fnt)
        p.setPen(QColor(255,255,255,240))
        p.drawText(banner.adjusted(0,-banner_h*0.1,0,-banner_h*0.2), Qt.AlignCenter,
                   self._game_result or "½-½")
        detail_map = {GAME_STALEMATE: "STALEMATE", GAME_INSUFFICIENT: "INSUFFICIENT\nMATERIAL", GAME_DRAW: "DRAW"}
        det_fnt = QFont("Segoe UI", max(6, int(banner_h*0.17))); p.setFont(det_fnt)
        p.setPen(QColor(255,255,255,190))
        p.drawText(banner.adjusted(0, banner_h*0.28, 0, 0), Qt.AlignCenter,
                   self._game_detail or detail_map.get(self._game_state, "DRAW"))
        p.setPen(QPen(QColor(200,170,40,35), 2.0)); p.setBrush(Qt.NoBrush)
        p.drawRoundedRect(QRectF(bx-1,by-1,bw+2,bh+2), self.CORNER_RADIUS+1, self.CORNER_RADIUS+1)
...
class PromotionWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        for pt, sym in {chess.QUEEN: "♕", chess.ROOK: "♖",
            ...
            b = QPushButton(sym); b.setFont(QFont("Segoe UI Symbol", 20))
            b.setFixedSize(48, 40); b.clicked.connect(lambda _, p=pt: self._pick(p))
            lay.addWidget(b); self._btns.append((pt, b))
        self.hide()
    ...
    def show_for_color(self, color):
        ...
        for pt, b in self._btns: b.setText(s[pt])
        self.show()

    def _pick(self, pt): self.piece_selected.emit(pt); self.hide()


# ════════════════════════════════════════════════════════════════════
#  VideoRenderer — renders full video frames using BoardRenderer
#  Thread-safe: no QWidget dependency
# ════════════════════════════════════════════════════════════════════
class VideoRenderer:
    """Renders a full video frame (board + eval bar + move list + overlays).

    Uses BoardRenderer instead of ChessBoardWidget, so it is safe to
    call from any thread.
    """

    def __init__(self, board_renderer, w=1920, h=1080, bg_color=QColor(30,30,32)):
        self.board_renderer = board_renderer
        self.w = w; self.h = h; self.bg_color = bg_color
        self.eval_cp = 0.0; self.move_text = ""
        self.white_name = "White"; self.black_name = "Black"
        self.overlays = []; self.move_list_text = []
        self.current_move_index = 0
        self.game_state = GAME_NORMAL; self.game_result = ""; self.game_detail = ""

    @staticmethod
    def _cp2r(cp):
        if abs(cp) >= 9000: return 1.0 if cp > 0 else 0.0
    ...
    def render(self):
        ...
        blk.setColorAt(0.0, QColor(62,62,70)); blk.setColorAt(0.5, QColor(48,48,55))
        ...
        if wh > 0:
            ...
            wg.setColorAt(0.0, QColor(232,228,218)); wg.setColorAt(0.4, QColor(240,237,228))
            ...
            if wh >= bsz:
                ...
                path = QPainterPath(); path.moveTo(ebx, wt)
                path.lineTo(ebx+ebw, wt); path.lineTo(ebx+ebw, by+bsz-5)
                path.quadTo(ebx+ebw, by+bsz, ebx+ebw-5, by+bsz)
                path.lineTo(ebx+5, by+bsz); path.quadTo(ebx, by+bsz, ebx, by+bsz-5)
                path.lineTo(ebx, wt); path.closeSubpath(); p.drawPath(path)
        ...
        for cp_val in range(-900, 901, 100):
            ...
            if by+4 > y or y > by+bsz-4: continue
            is_major = cp_val % 200 == 0; tw = 6 if is_major else 3
            p.setPen(Qt.NoPen); p.setBrush(QColor(255,255,255, 60 if is_major else 30))
            p.drawRect(QRectF(ebx+ebw-tw, y-0.5, tw, 1.0))
            if is_major and cp_val % 400 == 0 and cp_val != 0:
                fnt = QFont("Segoe UI", max(7, int(ebw*0.18))); p.setFont(fnt)
                p.setPen(QColor(170,170,190,130))
                p.drawText(QRectF(ebx+1, y-7, ebw-7, 12),
                           Qt.AlignLeft|Qt.AlignVCenter, f"{cp_val//100:+d}")

        # Eval bar game-state overlay or pill
        if self.game_state != GAME_NORMAL:
            ...
            efnt = QFont("Segoe UI", efsz, QFont.Bold); p.setFont(efnt)
            efm = p.fontMetrics(); etw = efm.horizontalAdvance(txt)+12
        ...
        if mw > 60:
            p.setBrush(QColor(40,40,40)); p.drawRoundedRect(mx, by, mw, bsz, 8, 8)
            p.setFont(QFont("Consolas", 14)); xo, yo, lh = 10, 15, 25
            for i, san in enumerate(self.move_list_text):
                p.setPen(QColor(100,180,255) if i == self.current_move_index else QColor(180,180,180))
                if i % 2 == 0:
                    p.drawText(QRectF(mx+xo, by+yo, 40, lh), Qt.AlignLeft, f"{i//2+1}.")
                    p.drawText(QRectF(mx+xo+40, by+yo, 70, lh), Qt.AlignLeft, san)
                else:
                    p.drawText(QRectF(mx+xo+120, by+yo, 70, lh), Qt.AlignLeft, san)
                if i % 2 == 1: yo += lh
                if yo > bsz - 20: break
        ...
        p.drawText(QRectF(bx_board, by+bsz+10, bsz/2, 40),
                   Qt.AlignLeft|Qt.AlignVCenter, self.white_name)
        p.drawText(QRectF(bx_board, by-50, bsz/2, 40),
    ...
    def _draw_video_game_state(self, p, ebx, by, ebw, bsz, bdy, wh):
        if self.game_state == GAME_CHECKMATE:
            ...
            p.setPen(Qt.NoPen)
            p.setBrush(QColor(25,160,55,40) if white_wins else QColor(190,35,35,40))
            p.drawRoundedRect(QRectF(ebx, by, ebw, bsz), 5, 5)
            center_y = by + bsz/2; bh_b = max(40, int(bsz*0.10))
            banner = QRectF(ebx-4, center_y-bh_b/2, ebw+8, bh_b)
            bg = QColor(25,140,55,220) if white_wins else QColor(190,35,35,220)
            p.setBrush(QColor(0,0,0,70)); p.drawRoundedRect(banner.adjusted(2,2,2,2), 6, 6)
            p.setBrush(bg); p.setPen(QPen(QColor(255,255,255,50), 0.8))
            p.drawRoundedRect(banner, 6, 6)
            fsz = max(8, int(bh_b*0.4)); p.setFont(QFont("Segoe UI", fsz, QFont.Bold))
            p.setPen(QColor(255,255,255))
            p.drawText(banner, Qt.AlignCenter, self.game_result or ("1-0" if white_wins else "0-1"))
            glow = QColor(25,200,60,50) if white_wins else QColor(220,40,40,50)
            p.setPen(QPen(glow, 2.0)); p.setBrush(Qt.NoBrush)
            p.drawRoundedRect(QRectF(ebx-2, by-2, ebw+4, bsz+4), 6, 6)
        elif self.game_state in (GAME_STALEMATE, GAME_DRAW, GAME_INSUFFICIENT):
            p.setPen(Qt.NoPen); p.setBrush(QColor(180,150,40,20))
            p.drawRoundedRect(QRectF(ebx, by, ebw, bsz), 5, 5)
            center_y = by + bsz/2
            ...
            banner = QRectF(ebx-4, center_y-bh_b/2, ebw+8, bh_b)
            bg = QColor(180,150,40,210) if self.game_state == GAME_STALEMATE else QColor(100,100,110,200)
            p.setBrush(QColor(0,0,0,60)); p.drawRoundedRect(banner.adjusted(2,2,2,2), 6, 6)
            p.setBrush(bg); p.setPen(QPen(QColor(255,255,255,45), 0.8))
            p.drawRoundedRect(banner, 6, 6)
            fsz = max(8, int(bh_b*0.35)); p.setFont(QFont("Segoe UI", fsz, QFont.Bold))
            p.setPen(QColor(255,255,255))
            p.drawText(QRectF(banner.x(), banner.y(), banner.width(), banner.height()*0.55),
                       Qt.AlignCenter, self.game_result or "½-½")
            detail_map = {GAME_STALEMATE: "STALEMATE", GAME_INSUFFICIENT: "INSUFF.", GAME_DRAW: "DRAW"}
            ...
            p.drawText(QRectF(banner.x(), banner.y()+banner.height()*0.5,
                       ...
                       Qt.AlignCenter, self.game_detail or detail_map.get(self.game_state, "DRAW"))
            p.setPen(QPen(QColor(200,170,40,30), 1.5)); p.setBrush(Qt.NoBrush)
            p.drawRoundedRect(QRectF(ebx-2, by-2, ebw+4, bsz+4), 6, 6)

    def _draw_video_result_banner(self, p, bx, by, bsz):
        banner_h = int(self.h * 0.06); banner_y = by + bsz + 55
        banner = QRectF(bx, banner_y, bsz, banner_h)
        if self.game_state == GAME_CHECKMATE:
            white_wins = self.eval_cp > 0 or self.game_result == "1-0"
            bg = QColor(25,140,55,210) if white_wins else QColor(190,35,35,210)
            txt = f"♔ CHECKMATE  {self.game_result or '1-0'}" if white_wins else f"♚ CHECKMATE  {self.game_result or '0-1'}"
        else:
        ...
        p.setBrush(bg); p.setPen(QPen(QColor(255,255,255,50), 1.0))
        p.drawRoundedRect(banner, 6, 6)
        fsz = max(10, int(banner_h*0.45)); p.setFont(QFont("Segoe UI", fsz, QFont.Bold))
        p.setPen(QColor(255,255,255,240)); p.drawText(banner, Qt.AlignCenter, txt)